...
aniosDisponibles: number[] = [];
 ...
 async cargarValores(): Promise<void> {
  try {
    ...
    // 🔹 Obtener TODOS los años reales
    this.aniosDisponibles = this.getAniosDisponibles(this.allValores);

    // 🔹 Inicializar gráfica anual con TODOS los años
    this.initEvolutionChart();

    // 🔹 Otras vistas (mensual, tabla, etc.)
...
getAniosDisponibles(valores: Valor[]): number[] {
  return [...new Set(
    valores
      .map(v => v.anio)
      .filter(anio => typeof anio === 'number')
  )].sort((a, b) => a - b);
}
 ...
 initEvolutionChart(): void {
  ...
  const partidos = this.getUniquePadridos();

  this.chartData = {
    labels: this.aniosDisponibles.map(a => a.toString()),
    datasets: partidos.map(partido => ({
      label: partido,
      data: this.getDatosAnualesRealesPorPartido(partido),
      borderColor: this.getPartidoColor(partido),
      backgroundColor: this.getPartidoColor(partido),
      borderWidth: 3,
      tension: 0.35,
      fill: false,
      pointRadius: 4,
      pointBackgroundColor: this.getPartidoColor(partido),
...
getDatosAnualesRealesPorPartido(partido: string): number[] {
  return this.aniosDisponibles.map(anio => {

    const valores = this.allValores.filter(v =>
      v.partidoNombre === partido &&
      v.anio === anio &&
      (!this.estadoSeleccionado || v.estadoId === this.estadoSeleccionado.codigo)
    );

    if (!valores.length) return 0;

    const suma = valores.reduce(
      (acc, v) => acc + (v.porcentaje ?? 0),
      0
    );
...
getDatosAnualesPorPartido(
  ...
  const aniosOrdenados = this.aniosDisponibles
    .sort((a, b) => a - b)
    .slice(-totalAnios);

  return aniosOrdenados.map(anio => {

    const valoresDelAnio = this.allValores.filter(v =>
      v.partidoNombre === partido &&
      v.estadoId === this.estadoSeleccionado.codigo &&
      v.anio === anio
    );

    if (!valoresDelAnio.length) {
      return 0;
    }

    // promedio anual del partido
    const promedio =
      valoresDelAnio.reduce((sum, v) => sum + (v.porcentaje || 0), 0) /
      valoresDelAnio.length;
